...
│   └── isotope/                       # 安定同位体実験
│       ├── sample_metadata.csv
│       ├── Summary_isotope.xlsx       # 元データ（gitignore）
│       └── ...
└── results/                           # 解析出力（実験 → クラスごとにサブフォルダ）
    ├── unlabeled/
    │   ├── SM/
    │   ├── Cer/
    │   └── ...
...
# 特定クラスのみ
python preprocess.py data/unlabeled/Summary_edit_20250226.xlsx SM Cer PC PE PI PS

# 安定同位体実験
python preprocess.py data/isotope/Summary_isotope.xlsx --experiment isotope

# シート名が異なる場合
python preprocess.py data/unlabeled/NewExperiment.xlsx --sheet シート名
```

**前提ファイル：** `data/{experiment}/sample_metadata.csv`
...
> 新しい実験では `data/{experiment}/` フォルダを作成し、`sample_metadata.csv` を置くだけでOK
...
python pipeline.py Cer --experiment isotope
```

→ `results/{experiment}/{クラス名}/` に figure (PNG) と stats (CSV) が自動生成される
